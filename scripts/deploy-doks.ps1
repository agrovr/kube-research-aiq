...
Write-Host "Connecting kubectl to DOKS cluster '$ClusterName'..."
doctl kubernetes cluster kubeconfig save $ClusterName
kubectl config current-context

if (-not $SkipIngress) {
    Write-Host "Installing or upgrading ingress-nginx..."
    ...
    Write-Host "Point DNS for '$HostName' at the ingress load balancer address before expecting TLS to become ready."
}
...
Write-Host "Deploying KubeResearch AIQ..."
$helmArgs = @(
    "upgrade",
    "--install",
    "kuberesearch",
...
Write-Host "Deployment submitted. Check public endpoint: https://$HostName"
Write-Host "Check certificate status with: kubectl -n $Namespace get certificate,ingress"
